  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between relative">
        ...
        {/* ========================================================= */}
        <nav className="hidden md:flex items-center gap-5 lg:gap-8 font-medium text-slate-200 text-sm lg:text-base mx-auto" data-purpose="main-navigation">
          {navLinks.map((link) => {
            ...
            return (
              <div key={link.name} className="relative py-2 flex flex-col items-center">
                ...
                {/* Active Cyan Line Indicator */}
                {isActive && (
                  <span className="absolute -bottom-1 w-full h-[2.5px] bg-[#00A3FF] rounded-full shadow-[0_0_10px_#00A3FF]" />
                )}
              </div>
            );
          })}
        ...
        <div className="flex items-center gap-3 shrink-0">
          <Link
            to="/contact"
            className="hidden sm:inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-gradient-to-r from-[#0070f3] via-[#0085FF] to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-bold text-xs sm:text-sm tracking-wide uppercase shadow-[0_4px_20px_rgba(0,112,243,0.5)] transition-all duration-200 group active:scale-95 border border-blue-400/30"
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg bg-slate-800 text-white border border-slate-700 focus:outline-none"
      ...
      {isOpen && (
        <div className="md:hidden bg-[#040A17] border-t border-slate-800 px-6 py-5 flex flex-col gap-3 text-sm font-medium text-slate-200">
          {navLinks.map((link) => {
            const isActive = location.pathname === link.path;
            return (
              <Link
                ...
                className={`py-2 transition-colors ${
                  isActive ? 'text-[#00A3FF] font-bold border-l-2 border-[#00A3FF] pl-3' : 'hover:text-white'
                }`}
              >
                {link.name}
              </Link>
          ...
          <div className="pt-3 border-t border-slate-800">
            <Link
              to="/contact"
              className="flex items-center justify-center gap-2 w-full py-3 rounded-full bg-[#0070f3] text-white font-bold text-xs uppercase tracking-wider shadow-lg"
            >
              <span>Get a Quote</span>
